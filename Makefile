...
NODE_VERSION := $(shell node -v 2>/dev/null | sed 's/^v//' | sed 's/-.*//' | awk -F. '{printf "%03d%03d%03d", $$1, $$2, $$3}' 2>/dev/null)
ifeq ($(shell test -n "$(NODE_VERSION)" && test "$(NODE_VERSION)" -lt "022018000"; echo $$?),0)
